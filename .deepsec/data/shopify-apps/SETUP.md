# Agent setup for `shopify-apps`

This is a deepsec scanning workspace. Project `shopify-apps` was just registered
(target: `..`). Setup is incomplete — `data/shopify-apps/INFO.md`
...
2. **Fill in `data/shopify-apps/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id shopify-apps
pnpm deepsec process --project-id shopify-apps
```
