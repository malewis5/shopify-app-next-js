...
Currently configured project: `shopify-apps` (target: `..`).
...
original init command. Checkpoints in `data/shopify-apps/setup/setup-state.json`
...
`--project-id shopify-apps` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/shopify-apps/`.
